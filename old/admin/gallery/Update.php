...
mysql_select_db($database_AirShow, $AirShow);
$query_rsPhotoCat = "SELECT * FROM tbl_gallerycategories ORDER BY CategoryName ASC";
...
$upd_tbl_galleryphotos = new tNG_update($conn_AirShow);
$tNGs->addTransaction($upd_tbl_galleryphotos);
// Register triggers
$upd_tbl_galleryphotos->registerTrigger("STARTER", "Trigger_Default_Starter", 1, "POST", "KT_Update1");
$upd_tbl_galleryphotos->registerTrigger("BEFORE", "Trigger_Default_FormValidation", 10, $formValidation);
$upd_tbl_galleryphotos->registerTrigger("END", "Trigger_Default_Redirect", 99, "Detail.php?PhotoID={PhotoID}");
$upd_tbl_galleryphotos->registerTrigger("AFTER", "Trigger_ImageUpload", 97);
// Add columns
$upd_tbl_galleryphotos->setTable("tbl_galleryphotos");
$upd_tbl_galleryphotos->addColumn("Category", "STRING_TYPE", "POST", "Category");
$upd_tbl_galleryphotos->addColumn("Description", "STRING_TYPE", "POST", "Description");
$upd_tbl_galleryphotos->addColumn("Photo", "FILE_TYPE", "FILES", "Photo");
$upd_tbl_galleryphotos->addColumn("PhotoAlt", "STRING_TYPE", "POST", "PhotoAlt");
$upd_tbl_galleryphotos->setPrimaryKey("PhotoID", "NUMERIC_TYPE", "GET", "PhotoID");
...
$objDynamicThumb1->setFolder("../../assets/gallery/{rstbl_galleryphotos.Category}/");
$objDynamicThumb1->setRenameRule("{rstbl_galleryphotos.Photo}");
...
?>
    <form method="post" id="form1" action="<?php echo KT_escapeAttribute(KT_getFullUri()); ?>" enctype="multipart/form-data">
      <table width="100%" cellpadding="2" cellspacing="0">
        <tr>
          <th width="17%" valign="top" class="tablecell">Category:</th>
          <td width="83%" valign="top" class="tablecell"><select name="Category" class="form" id="Category">
              <?php
do {  
...
$WARichTextEditor_1 = CreateRichTextEditor ("Description", "../../WA_iRite/", "100%", "200px", "Basic", "../custom/Update_Description6.js", "".$row_rstbl_galleryphotos['Description']  ."");
?><?php echo $tNGs->displayFieldHint("Description");?> <?php echo $tNGs->displayFieldError("tbl_galleryphotos", "Description"); ?> </td>
        </tr>
        <tr>
          <th valign="top" class="tablecell">PhotoAlt Text:</th>
          <td valign="top" class="tablecell"><input name="PhotoAlt" type="text" class="form" id="PhotoAlt" value="<?php echo KT_escapeAttribute($row_rstbl_galleryphotos['PhotoAlt']); ?>" size="32" />
        ...
        <tr>
          ...
          <td valign="top" class="tablecell"><input name="Photo" type="file" class="form" id="Photo" size="32" />
            <?php echo $tNGs->displayFieldError("tbl_galleryphotos", "Photo"); ?><br />
        ...
        <tr>
          <td valign="top">&nbsp;</td>
          <td valign="top" class="tablecell"><input name="KT_Update1" type="submit" class="form" id="KT_Update1" value="Update record" /></td>
        </tr>
      </table>
    </form>
